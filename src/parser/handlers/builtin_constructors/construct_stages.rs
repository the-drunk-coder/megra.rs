...
pub fn construct_stages(tail: &mut Vec<Expr>) -> Atom {
	...
	for (i, _) in labels.iter().enumerate() {
		...
		rules.push(Rule {
                    ...
                    symbol: labels[i][0],
                    probability: if cyclical {1.0 - pnext - pprev} else { 1.0 - pnext },
		});
		...
		if cyclical {
		    ...
		    duration_mapping.insert((labels[i][0], labels.last().unwrap()[0]), dur_ev.clone());
		}
		...
		rules.push(Rule {
                    source: labels[i].clone(),
                    symbol: labels[i][0],
                    probability: if cyclical {1.0 - pnext - pprev} else { 1.0 - pprev },
		});
		...
		if cyclical {
		    ...
		    duration_mapping.insert((labels[i][0], labels.first().unwrap()[0]), dur_ev.clone());
		}
