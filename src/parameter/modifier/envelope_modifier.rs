...
impl Modifier for EnvelopeModifier {         
	...
	let val = if self.current_from == self.current_to || self.done {
	    ...
	    let degree_increment = 90.0 / (self.current_steps - 1) as f32;
...
mod tests {
	...
	let values = vec![Parameter::with_value(0.0), Parameter::with_value(10.0), Parameter::with_value(0.0)];
	...
	let steps = vec![Parameter::with_value(10.0), Parameter::with_value(5.0), Parameter::with_value(10.0)];
	let values = vec![Parameter::with_value(0.0), Parameter::with_value(10.0), Parameter::with_value(5.0)];
